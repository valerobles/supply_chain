...
module {
    ...
    public func attempt<A>(item : A, matcher : Matcher<A>) : {
        #success;
        #fail : Text;
    } {
    ...
    public func describedAs<A>(msg : Text, matcher : Matcher<A>) : Matcher<A> = {
        ...
        describeMismatch = func(item : A, description : Description) = description.appendText(msg);
    };
    ...
    public func equals<A>(expected : T.TestableItem<A>) : Matcher<A> = {
        ...
        describeMismatch = func(item : A, description : Description) = description.appendText(expected.display(item) # " was expected to be " # expected.display(expected.item));
    };
    ...
    public func greaterThan<A <: Int>(expected : Int) : Matcher<A> = {
        ...
        describeMismatch = func(item : A, description : Description) = description.appendText(Int.toText(expected) # " was expected to be greater than " # Int.toText(item));
    };

    /// Matches values greater than or equal to `expected`
    public func greaterThanOrEqual<A <: Int>(expected : Int) : Matcher<A> = {
        matches = func(item : A) : Bool = item >= expected;
        describeMismatch = func(item : A, description : Description) = description.appendText(Int.toText(expected) # " was expected to be greater than or equal to " # Int.toText(item));
    };

    /// Matches values less than `expected`
    public func lessThan<A <: Int>(expected : Int) : Matcher<A> = {
        matches = func(item : A) : Bool = item < expected;
        describeMismatch = func(item : A, description : Description) = description.appendText(Int.toText(expected) # " was expected to be less than " # Int.toText(item));
    };

    /// Matches values less than or equal to `expected`
    public func lessThanOrEqual<A <: Int>(expected : Int) : Matcher<A> = {
        matches = func(item : A) : Bool = item <= expected;
        describeMismatch = func(item : A, description : Description) = description.appendText(Int.toText(expected) # " was expected to be less than or equal to " # Int.toText(item));
    };
    ...
    public func inRange<A <: Int>(lower : Int, upper : Int) : Matcher<A> = {
        ...
        describeMismatch = func(item : A, description : Description) = description.appendText(Int.toText(item) # " was expected to be in range [" # Int.toText(lower) # " .. " # Int.toText(upper) # "]");
    };
        ...
        describeMismatch = func(items : [A], description : Description) {
            if (items.size() != matchers.size()) {
                description.appendText(
                    ...
                    Nat.toText(items.size()) # " items and " #
                    Nat.toText(matchers.size()) # " matchers"
                );
                return;
            };
            for (ix in items.keys()) {
    ...
    public func isSome<A>() : Matcher<?A> = {
        ...
        describeMismatch = func(item : ?A, description : Description) = description.appendText("expected some value, but got `null`");
    };
    ...
    public func isNull<A>() : Matcher<T.TestableItem<?A>> = {
        ...
        describeMismatch = func(testable : T.TestableItem<?A>, description : Description) = Option.iterate<A>(
            testable.item,
            func(i) {
